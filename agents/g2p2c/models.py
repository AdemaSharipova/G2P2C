...
class ActorNetwork(nn.Module):
    ...
    def forward(self, s, feat, old_action, mode, is_training=False, activity_feat=None):
        ...
        if mode == 'forward':
            cgm_mu, cgm_sigma, cgm = self.GlucoseModel.forward(lstmOut, action.detach(), mode, activity_feat=activity_feat)
        else:
            cgm_mu, cgm_sigma, cgm = self.GlucoseModel.forward(lstmOut, old_action.detach(), mode, activity_feat=activity_feat)
    ...
    def horizon_error(self, s, feat, actions, real_glucose, mode, activity_feat=None):
        ...
        for i in range(0, len(actions)):
            ...
            extract_states, lstmOut = self.FeatureExtractor.forward(s, feat, mode, activity_feat=activity_feat)  # .detach()
            extract_states, lstmOut = extract_states.detach(), lstmOut.detach()

            cgm_mu, cgm_sigma, cgm_pred = self.GlucoseModel.forward(lstmOut, cur_action, mode, activity_feat=activity_feat)
...
class CriticNetwork(nn.Module):
    ...
    def forward(self, s, feat, action, cgm_pred=True, mode='forward', activity_feat=None):
        extract_states, lstmOut = self.FeatureExtractor.forward(s, feat, mode, activity_feat=activity_feat)
        value = self.ValueModule.forward(extract_states)
        cgm_mu, cgm_sigma, cgm = self.GlucoseModel.forward(lstmOut, action.detach(), mode, activity_feat=activity_feat) if cgm_pred else (
        None, None, None)
...
class ActorCritic(nn.Module):
    ...
    def predict(self, s, feat, activity_feat=None):
        s = torch.as_tensor(s, dtype=torch.float32, device=self.device)
        feat = torch.as_tensor(feat, dtype=torch.float32, device=self.device)
        mean, std, action, log_prob, a_cgm_mu, a_cgm_sigma, a_cgm = self.Actor(s, feat, None, mode='forward',
                                                                               is_training=self.is_testing_worker, activity_feat=activity_feat)
        state_value, c_cgm_mu, c_cgm_sigma, c_cgm = self.Critic(s, feat, action, cgm_pred=True, mode='forward', activity_feat=activity_feat)
        return (mean, std, action, log_prob, a_cgm_mu, a_cgm_sigma, a_cgm), (state_value, c_cgm_mu, c_cgm_sigma, c_cgm)

    def get_action(self, s, feat, activity_feat=None):
        (mu, std, act, log_prob, a_cgm_mu, a_cgm_sig, a_cgm), (s_val, c_cgm_mu, c_cgm_sig, c_cgm) = self.predict(s,
                                                                                                                 feat, activity_feat=activity_feat)
    ...
    def evaluate_actor(self, state, action, feat, activity_feat=None):  # evaluate batch
        action_mean, action_std, _, _, a_cgm_mu, a_cgm_sigma, _ = self.Actor(state, feat, action, mode='batch', activity_feat=activity_feat)
    ...
    def evaluate_critic(self, state, feat, action, cgm_pred):  # evaluate batch
        state_value, c_cgm_mu, c_cgm_sigma, _ = self.Critic(state, feat, action, cgm_pred, mode='batch', activity_feat=activity_feat)
