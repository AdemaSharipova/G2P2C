...
class Worker:
    def __init__(self, args, mode, patients, env_ids, seed, worker_id, device):
        ...
        self.log1_columns = ['epi', 't', 'cgm', 'meal', 'ins', 'rew', 'rl_ins', 'mu', 'sigma',
                             'prob', 'state_val', 'day_hour', 'day_min', 'activity_intensity', 'activity_duration']
    ...
    def init_env(self):
        with open('/Users/adema/VSProjects/RL/output.txt', 'a') as f:
            f.write('init_env')
    ...
    def rollout(self, policy):
        ...
        activity_vec = np.zeros((1, 2), dtype=np.float32)

        for n_steps in range(0, rollout_steps):
            policy_step = policy.get_action(self.cur_state, self.feat, activity=activity_vec)
            ...
            act_intensity = info.get('activity_intensity', 0.0)
            act_duration = info.get('activity_duration', 0.0)
            activity_vec = np.array([act_intensity, act_duration], dtype=np.float32).reshape(1, -1)
            ...
            if self.worker_mode == 'training':   # store -> rollout for training
                ...
                self.memory.store(self.cur_state, self.feat, policy_step['action'][0],
                                  reward, policy_step['state_value'], policy_step['log_prob'], scaled_cgm, self.counter, activity=activity_vec)
            # update -> state.
            ...
            self.episode_history[self.counter] = [self.episode, self.counter, state.CGM, info['meal'] * info['sample_time'],
                                                  ...
                                                  info['day_min'], info.get('activity_intensity', 0),
                                                  info.get('activity_duration', 0)]
            self.counter += 1
            ...
            if criteria:  # episode termination criteria.
                if self.worker_mode == 'training':
                    final_val = policy.get_final_value(s=self.cur_state, feat=self.feat, activity=activity_vec)
